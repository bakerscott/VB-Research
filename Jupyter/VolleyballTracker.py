...
class VolleyballTracker:
    ...
    def __init__(self, filePath):
        ...
        self.initialVideo = cv2.VideoCapture(filePath)

    def displayVideo(self):
        vid = self.initialVideo
        try:
            while True:
                (ret, frame) = vid.read()
                if not ret:
    ...
    def neverAgain(self):
        self.strinG = 'life comes at you fast'
        self.internalF(self.strinG)

    def internalF(self, sT):
        print('stuff:', sT)

    def lifeComesAtYouFast(self):
        type(self.BGgFList)

    def robustBackgroundFinder(self):
    ...
    ....'''
        ...
        cap = self.initialVideo
        vidLength = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))

        kernelColor = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3,
                3))
        fgbgColor = cv2.createBackgroundSubtractorMOG2()

        kernelGray = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3,
                3))
        ...
        for i in range(vidLength):
            ...
            fgmaskColor = cv2.morphologyEx(fgmaskColor, cv2.MORPH_OPEN,
                    kernelColor)

            fgmaskGray = fgbgGray.apply(grayFrame)
            fgmaskGray = cv2.morphologyEx(fgmaskGray, cv2.MORPH_OPEN,
                    kernelGray)
        ...
        for i in range(len(firstGrayFrame)):
            ...
            for pixel in blackSpaceInRow[0]:
                ...
                for j in range(1, len(grayFrameList)):
                    if blackenedPixels.get('{}, {}, {}'.format(j, i,
                            pixel)):
                        continue

                    pixelList.append(grayFrameList[j][i][pixel])
                    averagedPixel = averagedPixel \
                        + grayFrameList[j][i][pixel]
                    counter += 1
                ...
                for i in range(len(pixelList)):
                    if pixelList[i] < pixMax - 3 * pixSTD:
                        print('Max: {}, STD: {}, Diff: {}, Current Pixel: {}'.format(pixMax,
                                pixSTD, pixMax - pixSTD, pixelList[i]))
                        continue
        ...
        cap.release()
        '''
        self.cFList = colorFrameList
        self.gFList = grayFrameList
        self.FGcFList = fgColorList
        self.FGgFList = fgGrayList
        self.BGcFList = bgColorList
        self.BGgFList = bgGrayList
